<div class="section black-sec inner-padd">
  <div class="container">
    <div class="row">
      <div class="col-12">
        <div class="title-with-text">
          <div class="title h2 color-white animate-item fadeInUp">REDSTONE in numbers</div>
          <div class="text color-2 animate-item fadeInUp delay-1">
            Taken met een ster behoren tot ons profiel. Deze taken hebben ons prijzen gebracht in de Verenigde Koningrijk maar ook in de hele wereld. Het <span>REDSTONE</span> team staat altijd klaar voor nieuwe uitdagingen.
          </div>
        </div>
      </div>
    </div>
    <div class="row counters justify-content-center justify-content-lg-start animate-item fadeInUp delay-2">
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97%" stroke-width="1" text-anchor="middle">
                <tspan class="text-bold">500</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-none d-sm-block">
                <tspan dx="110" dy="-60" class="text-bold text-sup">+</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-sm-none">
                <tspan dx="60" dy="-30" class="text-bold text-sup">+</tspan>
              </text>
            </svg>
          </div>
          <small>Projecten</small>
        </div>
      </div>
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97%" stroke-width="1" text-anchor="middle">
                <tspan class="text-bold">60</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-none d-sm-block">
                <tspan dx="63" dy="-60" class="text-bold text-sup">+</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-sm-none">
                <tspan dx="32" dy="-30" class="text-bold text-sup">+</tspan>
              </text>
            </svg>
          </div>
          <small>Onze mensen</small>
        </div>
      </div>
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97%" stroke-width="1" text-anchor="middle">
                <tspan class="text-bold">300</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-none d-sm-block">
                <tspan dx="110" dy="-60" class="text-bold text-sup">+</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-sm-none">
                <tspan dx="60" dy="-30" class="text-bold text-sup">+</tspan>
              </text>
            </svg>
          </div>
          <small>websites</small>
        </div>
      </div>
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97%" stroke-width="1" text-anchor="middle">
                <tspan class="text-bold">50</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-none d-sm-block">
                <tspan dx="63" dy="-60" class="text-bold text-sup">+</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-sm-none">
                <tspan dx="32" dy="-30" class="text-bold text-sup">+</tspan>
              </text>
            </svg>
          </div>
          <small>prijzen</small>
        </div>
      </div>
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97" stroke-width="1" font-size="100" text-anchor="middle">
                <tspan class="text-bold">18</tspan>
              </text>
            </svg>
          </div>
          <small class="ml-0">Jaren op de markt</small>
        </div>
      </div>
      <div class="col-6 col-lg-4 counter">
        <div class="counter-block">
          <div class="svg-wrapper">
            <svg version="1.1" viewBox="0 0 100 100" width="100%" height="100%" preserveAspectRatio="xMidYMid meet">
              <text x="50%" y="97%" stroke-width="1" text-anchor="middle">
                <tspan class="text-bold">400</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-none d-sm-block">
                <tspan dx="110" dy="-60" class="text-bold text-sup">+</tspan>
              </text>
              <text x="100%" y="97%" stroke-width="1" text-anchor="middle" class="d-sm-none">
                <tspan dx="60" dy="-30" class="text-bold text-sup">+</tspan>
              </text>
            </svg>
          </div>
          <small>Happy customers</small>
        </div>
      </div>
    </div>
  </div>
</div>