<!DOCTYPE html>
<html lang="en">

<head>
  <?php include 'inc/_top.php';?>
  <title>REDSTONE :: Peeklogic detail</title>

</head>

<body style="--accent-clr: #288FCE;">
  <div id="loader-wrapper"></div>
  <div id="content-block">
    <?php include 'inc/_header.php';?>

    <main>

      <!-- PAGE NAV -->
      <div class="page-nav">
      </div>

      <!-- PROJECT ICONS -->
      <div class="project-icons">
        <a class="clutch" href="https://clutch.co/profile/redstone?page=1&project_cost=&review_type=&service_provided=&sort_by=date_DESC#review-1560968" target="_blank" rel="nofollow">
          <img class="icon" src="img/clutch.svg" alt="">
        </a>
      </div>

      <!-- BREADCRUMBS  * -->
      <div class="section sec-breadcrumbs">
        <div class="container-fluid">
          <ul class="breadcrumbs relative">
            <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
              <a href="index.php" itemprop="url">
                <span itemprop="title">Home</span>
              </a>
            </li>
            <li itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
              <a href="portfolio.php" itemprop="url">
                <span itemprop="title">Case Studies</span>
              </a>
            </li>
            <li class="active" itemscope="" itemtype="http://data-vocabulary.org/Breadcrumb">
              <span itemprop="title">Peeklogic</span>
            </li>
          </ul>
        </div>
      </div>

      <!-- PROJECT HERO SECTION  * -->
      <div class="section prj-hero nav-section">

        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="prj-hero-info">
                <div class="tags">
                  <span class="tag">marketing</span>
                  <span class="tag">technology</span>
                </div>
                <h1 class="h1 title">Peeklogic</h1>
                <a class="hero-link" href="https://www.peeklogic.com/" target="_blank" rel="nofollow">
                  <b>Check the project now</b>
                  <svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M20.2077 0H14.4828C14.0457 0 13.6914 0.354293 13.6914 0.791479C13.6914 1.22862 14.0457 1.58292 14.4828 1.58292H19.4162V6.51632C19.4162 6.95346 19.7705 7.30775 20.2077 7.30775C20.6448 7.30775 20.9992 6.95346 20.9992 6.51632V0.791479C20.9992 0.354293 20.6449 0 20.2077 0Z" />
                    <path d="M20.7675 0.231913C20.4583 -0.0773042 19.9576 -0.0773042 19.6484 0.231913L8.35691 11.5234C8.0477 11.8323 8.0477 12.3335 8.35691 12.6425C8.51125 12.797 8.71387 12.8744 8.91649 12.8744C9.11882 12.8744 9.32144 12.797 9.47606 12.6425L20.7675 1.35102C21.0767 1.04209 21.0767 0.540842 20.7675 0.231913Z" />
                    <path d="M17.5176 9.84043C17.0805 9.84043 16.7261 10.1947 16.7261 10.6319V19.4171H1.58292V4.27383H10.3681C10.8052 4.27383 11.1595 3.91954 11.1595 3.4824C11.1595 3.04525 10.8052 2.69092 10.3681 2.69092H0.791479C0.354293 2.69092 0 3.04521 0 3.4824V20.2085C0 20.6457 0.354293 21 0.791479 21H17.5176C17.955 21 18.309 20.6457 18.309 20.2085V10.6319C18.309 10.1947 17.9548 9.84043 17.5176 9.84043Z" />
                  </svg>
                </a>
              </div>
            </div>
          </div>
        </div>

        <div class="prj-hero-img">
          <picture>
            <source srcset="img/prj-detail/14/hero-img.webp" type="image/webp">
            <source srcset="img/prj-detail/14/hero-img.png" type="image/png">
            <img src="img/prj-detail/14/hero-img.png" alt="" loading="lazy">
          </picture>
        </div>

        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="row">
                <div class="col-lg-9 col-xl-7">
                  <div class="prj-hero-info m-0">
                    <div class="prj-hero-text">
                      The company <b>Peeklogic</b> turned to our specialists with the need to develop a simple, concise design for its marketing website. This website combines stylish and bright colors, dynamic animations, and the necessary functionality to promote products to the market. The high-quality work of our experts allowed the client to bring his company to a new level of business.
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-xl-5 text-lg-center">
                  <div class="service-item">
                    <div class="service-title">Services provided</div>
                    <ul>
                      <li><span>HTML</span></li>
                      <li><span>Development</span></li>
                      <li><span>Support</span></li>
                    </ul>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>

        <div class="spacer-xl"></div>
      </div>

      <!-- PROJECT FEATURES * -->
      <div class="section nav-section">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="title-with-text">
                <div class="title h2 w-100 animate-item fadeInUp">
                  Project features
                </div>
              </div>

              <div class="row ft-row animate-item fadeInUp anime-1">
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-1.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Speed optimization
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-51.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Animated menu
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-50.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Sign up for a consultation
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-13.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Background decor
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-5.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Multilingual
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-3.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Constructor page
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-6.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Order analytics
                    </div>
                  </div>
                </div>
                <div class="col-lg-3 col-md-4 col-6">
                  <div class="ft-item">
                    <div class="ft-item-icon">
                      <img src="img/features/ft-icon-8.svg" alt="" loading="lazy">
                    </div>
                    <div class="ft-item-title">
                      Filtering projects
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="spacer-xl"></div>
      </div>

      <!-- DEVICE SCREEN * -->
      <div class="section nav-section">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="title-with-text">
                <div class="title h2 w-100 animate-item fadeInUp">
                  Optimization for each device
                </div>
              </div>
              <div class="swiper-entry device-screen-slider animate-item fadeInUp anime-1">
                <div class="swiper-button-prev d-none d-xl-block"><i></i></div>
                <div class="swiper-button-next d-none d-xl-block"><i></i></div>
                <div class="swiper-container" data-options='{"spaceBetween": 15, "slidesPerView": 1, "breakpoints": {"575": {"slidesPerView": 2, "spaceBetween": 15},"991": {"slidesPerView": 3, "spaceBetween": 30},"1199": {"slidesPerView": 4, "spaceBetween": 40}, "1640": {"slidesPerView": 4, "spaceBetween": 70}, "1999": {"slidesPerView": 5, "spaceBetween": 70}}}'>
                  <div class="swiper-wrapper">
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-1.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-1.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-1.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-2.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-2.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-2.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-3.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-3.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-3.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-4.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-4.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-4.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-5.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-5.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-5.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-6.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-6.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-6.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                    <div class="swiper-slide">
                      <div class="device-phone">
                        <div class="device-screen">
                          <picture>
                            <source srcset="img/prj-detail/14/screen-img-7.webp" type="image/webp">
                            <source srcset="img/prj-detail/14/screen-img-7.jpg" type="image/jpg">
                            <img src="img/prj-detail/14/screen-img-7.jpg" alt="">
                          </picture>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="swiper-pagination swiper-pagination-relative"></div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="spacer-xl"></div>
      </div>

      <!-- SOLUTIONS * -->
      <div class="section nav-section">
        <div class="container-fluid">
          <div class="title-with-text">
            <div class="title h2 w-100 animate-item fadeInUp">
              Ready to go solutions
            </div>
          </div>
          <div class="row solution-row">
            <div class="col-12 d-none d-sm-block animate-item fadeInUp anime-1">
              <div class="solution-img solution-bg">
                <picture>
                  <source srcset="img/prj-detail/14/sol-img-1.webp" type="image/webp">
                  <source srcset="img/prj-detail/14/sol-img-1.png" type="image/png">
                  <img src="img/prj-detail/14/sol-img-1.png" alt="" loading="lazy">
                </picture>
              </div>
            </div>
            <div class="col-md-5">
              <div class="row solution-row">
                <div class="col-sm-6 col-md-12 animate-item fadeInUp">
                  <div class="solution-img">
                    <picture>
                      <source srcset="img/prj-detail/14/sol-img-2.webp" type="image/webp">
                      <source srcset="img/prj-detail/14/sol-img-2.png" type="image/png">
                      <img src="img/prj-detail/14/sol-img-2.png" alt="" loading="lazy">
                    </picture>
                  </div>
                </div>
                <div class="col-sm-6 col-md-12 animate-item fadeInUp">
                  <div class="solution-img">
                    <picture>
                      <source srcset="img/prj-detail/14/sol-img-4.webp" type="image/webp">
                      <source srcset="img/prj-detail/14/sol-img-4.png" type="image/png">
                      <img src="img/prj-detail/14/sol-img-4.png" alt="" loading="lazy">
                    </picture>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-md-7">
              <div class="row solution-row">
                <div class="col-12 animate-item fadeInUp anime-1">
                  <div class="solution-img">
                    <picture>
                      <source srcset="img/prj-detail/14/sol-img-3.webp" type="image/webp">
                      <source srcset="img/prj-detail/14/sol-img-3.png" type="image/png">
                      <img src="img/prj-detail/14/sol-img-3.png" alt="" loading="lazy">
                    </picture>
                  </div>
                </div>
                <div class="col-sm-6 animate-item fadeInUp anime-1">
                  <div class="solution-img">
                    <picture>
                      <source srcset="img/prj-detail/14/sol-img-5.webp" type="image/webp">
                      <source srcset="img/prj-detail/14/sol-img-5.png" type="image/png">
                      <img src="img/prj-detail/14/sol-img-5.png" alt="" loading="lazy">
                    </picture>
                  </div>
                </div>
                <div class="col-sm-6 animate-item fadeInUp anime-2">
                  <div class="solution-img">
                    <picture>
                      <source srcset="img/prj-detail/14/sol-img-6.webp" type="image/webp">
                      <source srcset="img/prj-detail/14/sol-img-6.png" type="image/png">
                      <img src="img/prj-detail/14/sol-img-6.png" alt="" loading="lazy">
                    </picture>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="spacer-xl"></div>
      </div>

      <!-- Tech used * -->
      <div class="section nav-section">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="row justify-content-center">
                <div class="col-xl-6">
                  <div class="title-with-text">
                    <div class="title h2 w-100 animate-item fadeInUp">
                      Technologies used
                    </div>
                  </div>
                </div>
                <div class="col-xl-6">
                  <div class="tech-wrap">
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-14.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-15.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-16.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-17.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-18.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-9.svg" loading="lazy" alt=""></div>
                    </div>
                    <div class="tech-item">
                      <div class="tech-img"><img src="img/tech/tech-img-7.svg" loading="lazy" alt=""></div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="spacer-xl"></div>
      </div>

      <!-- Client testimonial -->
      <div class="section nav-section">
        <div class="container">
          <div class="row justify-content-center">
            <div class="col-xl-11">
              <div class="row justify-content-center">
                <div class="col-xl-6">
                  <div class="title-with-text">
                    <div class="title h2 w-100 animate-item fadeInUp">
                      Client testimonial
                    </div>
                  </div>
                </div>
                <div class="col-md-10 col-lg-8 col-xl-6">
                  <div class="reviews-slider type2">
                    <div class="review-block">
                      <a class="full-link" href="https://clutch.co/profile/redstone?page=1&amp;project_cost=&amp;review_type=&amp;service_provided=&amp;sort_by=date_DESC#review-1560968" rel="nofollow" target="_blank"></a>
                      <div class="review-info">
                        <div class="review-quote"><img src="img/reviews/review-cl-2.jpg" alt=""></div>
                        <div class="review-title">Sergii Grushai</div>
                        <div class="review-category">CEO &amp; Founder of Peeklogic Salesforce Development Company</div>
                      </div>
                      <div class="review-content">
                        <div class="rating">
                          <img src="img/start-icon.svg" loading="lazy" alt="">
                          <img src="img/start-icon.svg" loading="lazy" alt="">
                          <img src="img/start-icon.svg" loading="lazy" alt="">
                          <img src="img/start-icon.svg" loading="lazy" alt="">
                          <img src="img/start-icon.svg" loading="lazy" alt="">
                        </div>
                        <div class="text">
                          <p>"It was a first-class service. We did not see anything that could be improved."</p>
                        </div>
                        <div class="review-price-block text">
                          <div class="verify-review"><img src="img/verified-icon.svg" loading="lazy" alt="verified icon">Verified</div>
                          <div class="verify-price"><img src="img/price-icon.svg" loading="lazy" alt="price icon">More than $ 30,000</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="spacer-xl"></div>
      </div>

      <!-- BRIEF -->
      <?php include 'inc/sections/_brief-2.php'; ?>

    </main>

    <?php include 'inc/_footer.php';?>
  </div>


  <div class="popup-wrapper" id="popups"></div>
  <?php include 'inc/_bottom.php';?>
</body>

</html>